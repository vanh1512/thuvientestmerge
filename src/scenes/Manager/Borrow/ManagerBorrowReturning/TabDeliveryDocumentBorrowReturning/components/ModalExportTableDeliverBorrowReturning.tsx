...
import TableDeliveryDocumentBorrowReturning from './TableDeliveryDocumentBorrowReturning';
...
export default class ModalExportTableDeliverBorrowReturning extends React.Component<IProps> {
		...
		return (
			<Modal
				...
				title={
					<Row>
						<Col span={12}>
							<h2>{L('danh_sach_giao_tai_lieu')}</h2>
						</Col>
						<Col span={12} style={{ textAlign: 'right' }}>
							<ActionExport
								isntHeaderReport={async () => await this.setState({ isHeaderReport: false })}
								isHeaderReport={async () => await this.setState({ isHeaderReport: true })}
								nameFileExport={'Danh_sach_giao_tai_lieu_ngay_' + moment().format('DD_MM_YYYY')}
