...
export default class TableDeliveryDocumentBorrowReturning extends React.Component<IProps> {
	state = {
		isLoadDone: true,
	...
	render() {
		...
		const columns: ColumnsType<BorrowReturningDto> = [
			...
			{
				title: L('NumberOfBorrowedDocument'), key: 'nr_borrowReturning', width: '8%',
				render: (text: string, item: BorrowReturningDto) => <div>{item.br_re_nr_document}</div>
			},
			{
				title: L('Status'), key: 'status_borrowReturning', width: '20%',
				render: ((text: string, item: BorrowReturningDto) => {
					if (is_printed !== undefined && is_printed) {
						return <div>{valueOfeBorrowReturningProcess(item.br_re_status)}</div>
					} else {
		...
		return (
			<Table className='centerTable'
				...
				rowClassName={(record, index) => (!!this.props.borrowReSelected && this.props.borrowReSelected.br_re_id == record.br_re_id) ? "bg-click" : "bg-white"}
