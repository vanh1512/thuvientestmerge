...
import { CreateMarc21Input } from '@src/services/services_autogen';
...
export default class ModalImportMarc21 extends React.Component<IProps>{
	state = {
		isLoadDone: false,
	}
			...
			readXlsxFile(item).then((rows) => {
				...
				if (rows != undefined && length > 1) {
					for (let i = 1; i < length; i++) {
						let itemCreate: CreateMarc21Input = new CreateMarc21Input();
						let item = rows[i];
						...
						//  Giới hạn item 1 tối đa 50 ký tự
							...
							itemCreate.mar_desc = item[2] != null ? item[2].toString() : "";
	...
	async createListLSC() {
		...
		confirm({
			...
			cancelText: L('huy'),
			async onOk() {
				await stores.marc21Store.createListMarc21(self.dataExcel);
	...
	render() {
		const columns = [
			...
			{ title: L('ma'), dataIndex: 'mar_code', key: 'mar_code', render: (text: string) => <div>{text}</div> },
			{ title: L('Description'), dataIndex: 'mar_desc', key: 'mar_desc', render: (text: string) => <div>{text}</div> },
		];
		return (
			<Modal
			...
			>
				<Row gutter={[8, 8]} style={{ fontSize: 16, fontWeight: "bold" }}>
					<Col {...cssColResponsiveSpan(24, 24, 24, 24, 15, 15)} style={{ textAlign: "left" }} >
						...
						<Button type="default" style={{ color: 'red', backgroundColor: 'floralwhite' }} title={L('du_lieu_mau')} target="_blank" href={process.env.PUBLIC_URL + "/sample_import/marc21_mau.xlsx"}>
