...
import { CreateDictionaryTypeInput } from '@src/services/services_autogen';
...
export default class ModalImportExcelDictionaryType extends React.Component<IProps>{
	state = {
		isLoadDone: false,
	}
	...
	dataExcel: CreateDictionaryTypeInput[] = [];
	onCancel = () => {
			...
			readXlsxFile(item).then((rows) => {
				...
				if (rows != undefined && length > 1) {
					for (let i = 1; i < length; i++) {
						let itemCreate: CreateDictionaryTypeInput = new CreateDictionaryTypeInput();
						let item = rows[i];
						...
						else if (item[1].toString().length > AppConsts.maxLength.name) {
							message.error(L('ten_khong_duoc_dai_qua_50_ky_tu'))
							break;
						}
						else {
							itemCreate.dic_ty_name = item[1].toString();
							itemCreate.dic_ty_desc = item[2] != null ? item[2].toString() : "";
	...
	async createListLSC() {
		...
		confirm({
			...
			cancelText: L('huy'),
			async onOk() {
				await stores.dictionaryTypeStore.createList(self.dataExcel);
	...
	render() {
		const columns = [
			...
			{ title: L('ten_tu_dien'), dataIndex: 'dic_ty_name', key: 'fie_name', render: (text: string) => <div>{text}</div> },
			{ title: L('Description'), dataIndex: 'dic_ty_desc', key: 'fie_desc', render: (text: string) => <div>{text}</div> },
		];
		return (
			<Modal
				...
				maskClosable={false}
				title={L("NHAP_DU_LIEU_LOAI_TU_DIEN")}
			>
				<Row gutter={[8, 8]} style={{ fontSize: 16, fontWeight: "bold" }}>
					<Col {...cssColResponsiveSpan(24, 24, 24, 24, 15, 15)} style={{ textAlign: "left" }} >
						...
						<Button type="default" style={{ color: 'red', backgroundColor: 'floralwhite' }} title={L('du_lieu_mau')} target="_blank" href={process.env.PUBLIC_URL + "/sample_import/loai_tu_dien.xlsx"}>
