...
import { CreateFieldsInput } from '@src/services/services_autogen';
...
export default class ModalImportField extends React.Component<IProps>{
	state = {
		isLoadDone: false,
	}
			...
			readXlsxFile(item).then((rows) => {
				...
				if (rows != undefined && length > 1) {
					for (let i = 1; i < length; i++) {
						let itemCreate: CreateFieldsInput = new CreateFieldsInput();
						let item = rows[i];
						...
						//  Giới hạn item 1 tối đa 50 ký tự
							...
							itemCreate.fie_name = item[1].toString();
							itemCreate.fie_desc = item[2] != null ? item[2].toString() : "";
	...
	async createListLSC() {
		...
		confirm({
			...
			cancelText: L('huy'),
			async onOk() {
				await stores.fieldsStore.createListField(self.dataExcel);
	...
	render() {
		const columns = [
			...
			{ title: L('FieldName'), dataIndex: 'fie_name', key: 'fie_name', render: (text: string) => <div>{text}</div> },
			{ title: L('Description'), dataIndex: 'fie_desc', key: 'fie_desc', render: (text: string) => <div>{text}</div> },
		];
		return (
			<Modal
				...
				maskClosable={false}
				title={L("NHAP_DU_LIEU_LINH_VUC_TAI_LIEU.")}
			>
				<Row gutter={[8, 8]} style={{ fontSize: 16, fontWeight: "bold" }}>
					<Col {...cssColResponsiveSpan(24, 24, 24, 24, 15, 15)} style={{ textAlign: "left" }} >
						...
						<Button type="default" style={{ color: 'red', backgroundColor: 'floralwhite' }} title={L('du_lieu_mau')} target="_blank" href={process.env.PUBLIC_URL + "/sample_import/linh_vuc_tai_lieu_mau.xlsx"}>
